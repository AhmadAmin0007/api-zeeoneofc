...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di https://wa.me/6289530585972 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di https://wa.me/6289530585972 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
    ...
    error: {
        status: false,
        creator: `${creator}`,
        message: 'sedang dilakukan perbaikan'
    }
}
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------AhmadGanteng'+'AHMAD--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'AhmadAmin0007') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AhmadAmin0007') return res.json(loghandler.invalidKey)
...
router.get('/yt/stalk', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AhmadAmin0007') return res.json(loghandler.invalidKey)
	if(!search) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter yt"})
       fetch(encodeURI(`https://api.lolhuman.xyz/api/ytchannel?apikey=17d5183ef3f4ddaf16a91557&query=${yt}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
